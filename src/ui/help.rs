...
pub fn render_help(config: &Config, area: Rect, buf: &mut Buffer) {
  ...
  let lines: Vec<Line> = vec![
    ...
    section_line("Search & Filter"),
    entry_line(&lookup, Action::SearchStart, "Search by name"),
    entry_line(&lookup, Action::SizeFilterStart, "Filter by size"),
    entry_line(&lookup, Action::DateFilterStart, "Filter by date"),
    entry_line(&lookup, Action::SavedSearchesOpen, "Saved searches"),
    hardcoded_line("Enter", "Confirm filter"),
    hardcoded_line("Esc", "Cancel filter"),
